                    ...
                    @for($row = 1 ; $row <= $room->rows ; $row++)
                        <tr>
                            ...
                            <td class="text-center">
                                ...
                                <!-- <p>enabled</p> -->
                                <?php
                                    $currentSeat = $room->seats->filter(function($seat) {
                                        return (($seat->col == $tcol) &&($seat->row == $row));
                                    });
                                    $count = 0;
                                    foreach($RegistRecords as $RegistRecord) {
                                        if ($currentSeat) {
                                            if ($RegistRecord->seat_id == $currentSeat->id) { // 時間に関するフィルターはここに入れる
                                                $count++;
                                            }
                                        }
                                    }
